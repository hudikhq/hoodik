...
import { uint8 } from '.'
import { init, chacha_decrypt, chacha_encrypt, chacha_generate_key } from './wasm'
...
export async function encrypt(data: Uint8Array, key: Key): Promise<Uint8Array> {
  await init()
  const ciphertext = chacha_encrypt(key, data)
...
export async function decrypt(ciphertext: Uint8Array, key: Key): Promise<Uint8Array> {
  await init()
  const plaintext = chacha_decrypt(key, ciphertext)
