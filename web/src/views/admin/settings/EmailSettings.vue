  ...
  <CardBox :class="props.class">
    <div class="-mx-4 -mt-4 px-6 py-5 border-b border-paper-200 dark:border-brownish-700/50 rounded-t-2xl">
      <div class="flex items-center gap-2 mb-3">
        <BaseIcon :path="mdiEmail" :size="14" class="text-brownish-400 dark:text-brownish-50" />
        <p class="text-xs font-semibold text-brownish-400 dark:text-brownish-50">{{ $t('admin.email.configuration') }}</p>
      </div>
      <p class="text-sm text-brownish-400 dark:text-brownish-50 leading-relaxed">
    ...
    <div class="-mx-4 -mb-4 px-6 py-5 rounded-b-2xl">
      <div v-if="testSuccess" class="rounded-lg bg-greeny-500/10 border border-greeny-500/30 px-4 py-3 mb-3">
        <p class="text-sm text-greeny-500 dark:text-greeny-300">{{ testSuccess }}</p>
      </div>

      <div v-if="testError" class="rounded-lg bg-redish-500/10 border border-redish-500/30 px-4 py-3 mb-3">
        <p class="text-sm text-redish-400 dark:text-redish-100">{{ testError }}</p>
      </div>
      ...
      <div class="p-3 rounded-xl bg-paper-200 dark:bg-brownish-700/20 border border-paper-200/50 dark:border-brownish-700/30">
        <div class="flex items-center justify-between gap-4">
          <div class="min-w-0">
            ...
            <p class="text-xs text-brownish-400 dark:text-brownish-50 mt-0.5">{{ $t('admin.email.sendsToAccount') }}</p>
          </div>
          <div class="flex items-center gap-3 shrink-0">
            <span v-if="testingEmail" class="text-xs text-brownish-400 dark:text-brownish-50 animate-pulse">{{ $t('admin.email.sending') }}</span>
            <BaseButton
              color="info"
              :small="true"
