  ...
  <CardBox :class="props.class" v-if="user">
      ...
      <div class="flex items-center gap-4">
        ...
        <div class="min-w-0 flex-1">
          ...
          <div class="flex flex-wrap items-center gap-x-2.5 gap-y-1 mt-1.5">
            ...
            <span v-else class="inline-flex items-center text-xs bg-paper-100 dark:bg-brownish-700 text-brownish-400 dark:text-brownish-300 px-2.5 py-0.5 rounded-full">
        ...
        <div class="flex items-center justify-between gap-4 p-3 rounded-xl bg-paper-200 dark:bg-brownish-700/20 border border-paper-200/50 dark:border-brownish-700/30">
          <div class="min-w-0">
            <div class="flex items-center gap-2">
              ...
              <span v-else class="inline-flex items-center text-xs bg-paper-100 dark:bg-brownish-700 text-brownish-400 dark:text-brownish-300 px-2 py-0.5 rounded-full">
    ...
    <div class="-mx-4 -mb-4">
      <button
        ...
        class="w-full flex items-center justify-between px-6 py-3.5 text-xs font-semibold uppercase tracking-wider text-brownish-400 dark:text-brownish-100 hover:bg-paper-50 dark:hover:bg-brownish-700/30 transition-colors rounded-b-2xl"
      >
        <div class="flex items-center gap-2">
      ...
      <div v-if="showDetails" class="px-6 pb-5 space-y-3 border-t border-paper-200 dark:border-brownish-700/50 pt-4">
        ...
        <div class="flex items-start gap-3">
          ...
          <div class="flex-1 min-w-0 flex items-start gap-2">
            <code class="flex-1 font-mono text-xs break-all text-brownish-400 dark:text-brownish-300 leading-relaxed bg-paper-50 dark:bg-brownish-900/50 px-2.5 py-1.5 rounded-lg">{{ user.pubkey }}</code>
            <button
              @click="copyToClipboard(user.pubkey, 'pubkey')"
              class="shrink-0 mt-1 p-1 rounded-md text-brownish-400 hover:text-brownish-900 dark:hover:text-white hover:bg-paper-100 dark:hover:bg-brownish-700 transition-colors"
        ...
        <div class="flex items-start gap-3">
          ...
          <div class="flex-1 min-w-0 flex items-start gap-2">
            <code class="flex-1 font-mono text-xs break-all text-brownish-400 dark:text-brownish-300 bg-paper-50 dark:bg-brownish-900/50 px-2.5 py-1.5 rounded-lg">{{ user.fingerprint }}</code>
            <button
