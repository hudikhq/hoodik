 ...
 */
export async function download(file: ListAppFile, progress?: ProgressFunction): Promise<void> {
 ...
 */
async function start(file: ListAppFile, progress?: ProgressFunction): Promise<Uint8Array> {
  ...
  const results = await asyncPool(
    ...
    async (file: ListAppFile, i: number) => {
      const content = await http.downloadChunk(file, i)
